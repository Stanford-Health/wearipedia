...
def create_syn_data(start_date, end_date):
    ...
    for d in dates:
        ...
        coordinates = [(coordinate[0]+np.random.normal(-0.005, 0.005),
                        coordinate[1]+np.random.normal(-0.005, 0.005)) for x in range(12)]
        ...
        entry = pd.DataFrame.from_dict({'name': name_generator(d), 'id': id_generator(d), 'start_date': d.strftime('%Y-%m-%dT%H:%M:%SZ'), 'distance': distance, 'moving_time': moving_time_generator(d), 'elapsed_time': elapsed_time_generator(d),
