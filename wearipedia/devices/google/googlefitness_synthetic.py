...
def create_syn_data(start_date, end_date, bucketByTime):
    ...
    for d in dates:
        for iter in range(int(iter_count)):
            ...
            startmillis = int(d.timestamp()) * 1000 + iter * bucketByTime
            ...
            def syn_sleep_duration(x): return np.round(
                np.random.normal(8, 3), 1)
            ...
            def syn_heart_minutes(x): return np.round(
                np.random.normal(90, 30), 1)
            ...
            def syn_blood_glucose(x): return np.round(
                np.random.normal(100, 20), 1)

            def syn_activity(x): return np.round(np.random.normal(100, 50))

            def syn_distance(x): return np.round(
                np.random.normal(3000, 2000), 1)
