...
def create_syn_data(start_date,end_date):
    ...
    syn_calories = lambda x: np.round(np.random.normal(2500, 200),1)
    syn_carbs = lambda x: np.round(np.random.normal(250, 75),1)
    syn_fat = lambda x: np.round(np.random.normal(75, 25),1)
    syn_protein = lambda x: np.round(np.random.normal(100, 75),1)
    syn_sodium = lambda x: np.round(np.random.normal(2300, 500),1)
    syn_sugar = lambda x: np.round(np.random.normal(75, 25),1)
...
'Dips', 'Deadlift, Straight Leg','Decline Bench Press','Decline Dumbbell Fly',
 ...
 'Vertical Leg Lift, Knees Straight (Dip Bars)']
    ...
    for day in dates:
        ...
        # We will randomly select between 1 and 3 cardio exercises
        random_exercises = np.random.choice(exercises_cardio, cardio_count, replace=False)
        ...
        # We will randomly select between 1 and 10 strength exercises
        random_exercises = np.random.choice(exercises_strength, exercise_count, replace=False)
