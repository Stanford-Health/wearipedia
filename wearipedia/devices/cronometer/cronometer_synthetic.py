...
def create_syn_data(start_date, end_date):
    ...
    path = os.path.abspath(os.path.join(
        os.path.dirname(__file__), 'servings.csv'))
    ...
    for d in dates:
        ...
        biometrics.append({'Day': d.strftime('%Y-%m-%d'), 'Metric': 'Weight',
                          'Unit': 'kg', 'Amount': syn_weight+np.random.uniform(-2, 2)})
