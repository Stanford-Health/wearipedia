...
def create_syn_data(start_date, end_date):
    ...
    def random_duration(x): return np.random.randint(900000, 7200000)
    ...
    def sleep_time(): return 'T'+str(np.random.randint(20, 24))+':'+str(np.random.randint(0, 60)).zfill(2)+':'+str(np.random.randint(0, 60)).zfill(2) + \
        '.'+str(np.random.randint(0, 1000)).zfill(3)+'-'+str(np.random.randint(0,
                                                                               24)).zfill(2)+':'+str(np.random.randint(0, 60)).zfill(2)

    def wake_time(): return 'T'+str(np.random.randint(5, 10)).zfill(2)+':'+str(np.random.randint(0, 60)).zfill(2)+':'+str(np.random.randint(0, 60)
                                                                                                                          ).zfill(2)+'.'+str(np.random.randint(0, 1000)).zfill(3)+'-'+str(np.random.randint(4, 8)).zfill(2)+':'+str(np.random.randint(0, 60)).zfill(2)

    def activity_time(d): return d.strftime('%Y-%m-%d') + 'T'+str(np.random.randint(5, 22)).zfill(2)+':'+str(np.random.randint(0, 60)).zfill(2)+':'+str(
        np.random.randint(0, 60)).zfill(2)+'.'+str(np.random.randint(0, 1000)).zfill(3)+'-'+str(np.random.randint(4, 8)).zfill(2)+':'+str(np.random.randint(0, 60)).zfill(2)

    # Continutity of sleep
    def continuity(x): return np.round(np.random.rand()*2, 1)
    ...
    for d in dates:
        ...
        current_sleep = {
            ...
            'sleepEndTime': (d+pd.Timedelta(days=1)).strftime('%Y-%m-%d') + wake_time(),
            'sleepStartOffset': 0, 'sleepEndOffset': 0,
            'sleepRating': None,
        ...
        total_sleep_time = pd.to_datetime(
            current_sleep['sleepEndTime'][:19]) - pd.to_datetime(current_sleep['sleepStartTime'][:19])
        ...
        curr_id_traning = [
            {
                ...
                'Max pace (min/mi)': distance/(activity['distance']/3600000) if activity['sportId'] != 15 else None,
